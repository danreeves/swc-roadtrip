...
Counter.prototype._tick = function _tick (currentTime) {
...
Counter.prototype._ease = function _ease (t, b, c, d) {
...
Google.KEY = 'AIzaSyCxOAAdN7byIfuOUv2X6Ww5CBNab0y3EDs';
Google.load(function (google) {
    ...
    fetch(((document.location) + "data.json"))
        ...
        .then(function (json) { return json.map(function (loc) {
            var lat = loc.lat;
            var lng = loc.lon;
        ...
        .then(function (json) { return json.sort(function (a, b) {
            var aDist = haversine_1(a, scilly);
            var bDist = haversine_1(b, scilly);
        ...
        .then(function (json) {
            ...
            var journeys = index(json.reverse(), 9)
                .map(function (journey, i, arr) {
                    if (i < arr.length - 1) {
                        return journey.concat( [arr[i + 1][0]]);
                    }
                    return journey;
                });
            var distance = 0;
            ...
            journeys.forEach(function (journey, i) {
                var origin = journey[0];
                var destination = journey[journey.length - 1];
                var waypoints = journey
                    .slice(1, journey.length - 2)
                    .map(function (location) { return ({ location: location }); });
